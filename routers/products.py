...
async def add_product(request: AddProductRequest,aconn = Depends(get_db), user = Depends(get_current_user)):
    ...
    product_exists = await fetch_prodcut_by_barcode(request.barcode)
    
    if product_exists is None:
        master_products_id = await add_to_master_table(aconn, request.name, request.category, request.barcode)
    else: 
        master_products_id = product_exists['master_products_id']
    ...
    response = await add_to_products_table(aconn, user['business_id'], master_products_id, price, stock, custom_name, custom_category)
    ...
    return remove_custom_placeholders(response)
...
async def modify_product(business_product_id: str, request: ModifyProductRequest, aconn = Depends(get_db), user = Depends(get_current_user)):
    ...
    return await modify_product_by_id(aconn, business_product_id, user['business_id'], price, stock_quantity, custom_name, custom_category)
...
async def modify_product(business_product_id: str, aconn = Depends(get_db), user = Depends(get_current_user)):
    '''Deletes a Product'''
    return await delete_product_by_id(aconn, business_product_id, user['business_id'])
